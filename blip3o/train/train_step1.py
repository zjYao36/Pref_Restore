...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        model.initialize_vision_tokenizer(model_args, tokenizer=tokenizer) # 给tokenizer新增image token，scale token, image token, start_end token
        ...
        for name, param in model.named_parameters(): # sana不训练保持原本生成能力
            if "sana" in name:
