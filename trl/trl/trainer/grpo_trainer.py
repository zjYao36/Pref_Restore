...
from blip3o.model import blip3oQwenForGRPOLM
...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            model = blip3oQwenForGRPOLM.from_pretrained(model_id, torch_dtype=torch.bfloat16)
        ...
        if self.beta == 0.0:
            ...
            self.ref_model = blip3oQwenForGRPOLM.from_pretrained(model_id, torch_dtype=torch.bfloat16)
    ...
    @profiling_decorator
    def _calculate_rewards(self, inputs, prompts, generated_images):
        ...
        for img in generated_images:
            ...
            img_batch.append(img_bgr)
        
        results = ocr_model.predict(img_batch)
        ...
        for i, (prompt, result) in enumerate(zip(prompts, results)):
            ...
            rewards_per_func[i, 0] = 1 - dist / (len(prompt))



        max_idx = torch.argmax(rewards_per_func[:, 0]).item()
        best_img = generated_images[max_idx]
        best_img.save("highest_reward_image_final.png")
    ...
    def _generate_and_score_completions(
        ...
        for item in inputs:
            ...
            messages = [
                ...
                {"role": "user", "content": f"Please generate image based on the following caption: {item['prompt']}"}
            ]

            # messages = [
            #     {"role": "system", "content": "You are a helpful assistant."},
            #     {"role": "user", "content": "Please generate image based on the following caption: A clear image of a blackboard with a clean, dark green surface and the word 'Hello' written precisely and legibly in the center with bold, white chalk letters."}
            # ]
        ...
        if self.use_vllm:
            ...
            with (
                ...
                prompt_completion_ids, generated_images = unwrapped_model.generate_images(prompt_ids, prompt_mask, max_new_tokens=self.generation_config.max_new_tokens, do_sample=True,)
        ...
        rewards_per_func = self._calculate_rewards(inputs, original_prompts, generated_images)
